...
username = 'groupdadminlogin'  
password = 'cV2Mwql!5T'  
driver = '{ODBC Driver 18 for SQL Server}'

def save_to_db(url, title, meta, header, sections, footer):
    """Insert scraped data into Azure SQL Database
    This takes the scraped data and saves it into the database through calls later in the code.
    """
    try:
        conn = pyodbc.connect(
        ...
        sections_json = json.dumps(sections) #Uses json.dumps to convert sections to a JSON string

        # Insert or update data
        # Need to have create table here in case it doesn't exist
        cursor.execute("""
            MERGE INTO WebScrapedData AS target
            USING (SELECT ? AS url) AS source
            ON target.url = source.url
            WHEN MATCHED THEN 
                UPDATE SET title=?, meta=?, header=?, sections=?, footer=?
            WHEN NOT MATCHED THEN 
                INSERT (url, title, meta, header, sections, footer)
                VALUES (?, ?, ?, ?, ?, ?);
        """, url, title, meta_json, header, sections_json, footer, url, title, meta_json, header, sections_json, footer)
...
def scrape_page(url):
    ...
    # Extracting fields for DB storage
    # Here is second change from original scraper.py
    title = structured_data.get("title", "")
    meta = structured_data.get("meta", {})
    header = structured_data.get("header", "")
    sections = structured_data.get("sections", [])
    footer = structured_data.get("footer", "")

    # Save the scraped data into Azure SQL
    save_to_db(url, title, meta, header, sections, footer)

    # Extract internal links, removes scripts and styles
    soup = BeautifulSoup(html, 'html.parser')
